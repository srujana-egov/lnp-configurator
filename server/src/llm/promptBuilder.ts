...
const DOMAIN_RULES: Record<RoutableDomain, string> = {
  ...
  overallConfiguration: `This mirrors the real product's own step exactly — a genuine 8-question flow (confirmed against the real screens, not approximated). Ask in this order, one guided question at a time (same hard rule as every domain — never invent an answer the user didn't give), whenever nothing more urgent is pending:
...
3. Only if Renewal is enabled: how many days before expiry should a reminder go out, how many days of grace period after expiry, and how should renewals be approved? suggestedReplies for approval: ["Auto-approve every renewal", "Auto-approve only if nothing changed", "Always send renewals through the approval workflow"].
4. How many levels does the category structure have (1, 2, or 3)? suggestedReplies: ["Just 1 level", "2 levels", "3 levels"].
5. What should each level be called (e.g. "Category", then "Sub-category", then "Type" if more than one level)?
6. What are the real category values for each level? Never default to a generic starter list (the real product pre-fills common examples like Retail Shop/Food & Beverage/Healthcare, but that is the real product's own behavior, not something this extraction should imitate) — only capture names the conversation actually states.
7. What format should new-application IDs follow (e.g. "BL-YYYY-NNNNN"), does the renewal ID format differ, and should the issued licence's ID match the application's ID?

If the conversation jumps straight to a later question (e.g. describing renewal rules before saying whether Renewal is even enabled), ask #1 first — everything from #3 onward depends on it.`,
